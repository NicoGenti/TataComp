...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    // Cache static assets for 1 year (immutable strategy for versioned/hashed files)
    // HTML files are excluded and served with no-cache to enable SPA routing
    app.use(express.static(distPath, {
      maxAge: "1y",
      immutable: true,
      setHeaders: (res, filePath) => {
        // Don't cache HTML files to enable SPA routing and ensure fresh content
